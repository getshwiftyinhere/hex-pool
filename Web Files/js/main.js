...
function ShowTornado(){
  document.getElementById("tornadoCash").innerHTML = tornadoCash;
}
...
function UpdateRate() {
  ...
  var orderType;
  var escrowType = document.querySelector('input[name="escrowType"]:checked').value;
  var rateInput = document.getElementById("rate");
  var rate = hexInput.value / ethInput.value;
  if (isNaN(rate)) {
    rateInput.innerHTML = "N/A";
  } else {
    if (rate % 10 == 0) {
      rateInput.innerHTML = rate;
    } else {
      rateInput.innerHTML = toFixedMax(rate, decimals/2);
    }
    //update order info
    if (escrowType == 1) {
      var table = document.getElementById("sellTable");
      for (var i = 0; i < table.lastElementChild.childElementCount; i++) {
        var elem = table.lastElementChild.children[i];
        var rateElems = document.getElementsByClassName("tableRate");
        for (var j = 0; j < rateElems.length; j++) {
          if (rateElems[j].parentNode == elem) {
            console.log(rateElems[j].innerHTML);
            if (parseInt(rateElems[j].innerHTML) > rate) {
              successMessage("Someone's selling at a higher rate! <a style='color:limegreen' href='#about'>See sellers below...</a>");
            }
          }
        }
      }
      orderType = "BUYING";
      color = "green";
      escrowAmount = ethInput.value;
      currency = "ETH";
      approveNotice = "";
    } else if (escrowType == 0) {
      var table = document.getElementById("buyTable");
      for (var i = 0; i < table.lastElementChild.children.length; i++) {
        var elem = table.lastElementChild.children[i];
        var rateElems = document.getElementsByClassName("tableRate");
        for (var j = 0; j < rateElems.length; j++) {
          if (rateElems[j].parentNode == elem) {
            console.log(rateElems[j].innerHTML);
            if (parseInt(rateElems[j].innerHTML) < rate) {
              successMessage("Someone's buying at a lower rate! <a style='color:limegreen' href='#about'>See buyers below...</a>");
            }
          }
        }
      }
      orderType = "SELLING";
      color = "red";
      escrowAmount = hexInput.value;
      currency = "HEX";
      approveNotice = "Note: "+ escrowAmount +" HEX approval required.";
    }
    orderInfo.innerHTML = "You are <b style='color:" + color + "'>" + orderType + "</b> <b>" + hexInput.value + " HEX</b> " + "for <b>" + ethInput.value + " ETH</b> @ <b>" + toFixedMax(rate, 2) + " HEX per ETH</b><br/><span style='font-size:22px;'><i style='color:black' class='fa fa-info-circle'></i>&nbsp<b>" + escrowAmount + " " + currency + " to be escrowed.<br/><span style='font-size:16px;'>"+approveNotice+"</span></b></span>";
    //check rate against open offers and notify user(within % threshold)
  }
}
